...
const ModeratorQueueDetailsPage: React.FC = () => {
  ...
  const [settings, setSettings] = useState<
    QueueSettingsResponse["settings"] | null
  >(null);
  const [metrics, setMetrics] = useState<
    QueueMetricsResponse["metrics"] | null
  >(null);
  ...
  const handleInputChange = (
    field: keyof NonNullable<typeof editedSettings>,
  ...
  const isFormValid =
    editedSettings?.maxQueueSize !== undefined &&
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      ...
      try {
        ...
        try {
          const res = await queuesApi.getQueueSettings(
            queueId,
            authId,
            maxHash
          );
        ...
        try {
          const res = await queuesApi.getQueueGraphics(
            queueId,
            authId,
            maxHash
          );
          ...
          if (graphics) {
            ...
            const timeMap = new Map<
              string,
              { time: string; inQueue: number; served: number }
            >();
            ...
            waitingTimeData.forEach((item) => {
              if (item.time && item.waitingTime !== undefined) {
                const entry = timeMap.get(item.time) || {
                  time: item.time,
                  inQueue: 0,
                  served: 0,
                };
                const servedInPeriod = Math.max(
                  0,
                  Math.round(item.waitingTime * servedPerMinute)
                );
    ...
    <Container style={{ minHeight: "100vh", padding: 0 }}>
      ...
      <Flex direction="column" align="center" style={containerStyle}>
        ...
        {isDesktop ? (
          <Flex
            direction="row"
            style={{ width: "100%", gap: "24px", flexWrap: "wrap" }}
          >
              ...
              {isEditing && editedSettings ? (
                ...
                >
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("maxQueueSize", e.target.value)
                    }
                    style={{
                      padding: "8px 12px",
                      borderRadius: "8px",
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleInputChange("arrivalGracePeriod", e.target.value)
                    }
                    style={{
                      padding: "8px 12px",
                      borderRadius: "8px",
                ...
                >
                  ...
                  >
                    ...
                    <strong>{settings.maxQueueSize}</strong>
                  </div>
                  <div
                    style={{ display: "flex", justifyContent: "space-between" }}
                  >
                    <span>Льготный период:</span>{" "}
                    <strong>{settings.arrivalGracePeriod} мин</strong>
                  </div>
                  <div
                    style={{ display: "flex", justifyContent: "space-between" }}
                  >
                    ...
                    <strong
                      style={{
                        color: settings.isActive ? "#00AA00" : "#CC0000",
                      }}
                    >
            ...
            >
              <Typography.Title
                style={{ fontSize: "16px", margin: "0 0 12px" }}
              >
                Метрики
              </Typography.Title>
              {metrics ? (
                ...
                >
                  ...
                  >
                    <span>В очереди:</span>{" "}
                    <strong>{metrics.entriesInTheQueue}</strong>
                  </div>
                  <div
                    style={{ display: "flex", justifyContent: "space-between" }}
                  >
                    <span>Ожидание:</span>{" "}
                    <strong>{metrics.waitingTime} мин</strong>
                  </div>
                  <div
                    style={{ display: "flex", justifyContent: "space-between" }}
                  >
                    <span>Обслужено:</span>{" "}
                    <strong>{metrics.numberOfServedMembers}</strong>
                  </div>
                  <div
                    style={{ display: "flex", justifyContent: "space-between" }}
                  >
                    <span>Обслуживание:</span>{" "}
                    <strong>{metrics.serviceTime} мин</strong>
                  </div>
                  <div
                    style={{ display: "flex", justifyContent: "space-between" }}
                  >
            ...
            >
              <Typography.Title
                style={{ fontSize: "16px", margin: "0 0 16px" }}
              >
                Динамика за день
              </Typography.Title>
              {chartData ? (
          ...
          <>
            ...
            >
              <Flex
                ...
                style={{ marginBottom: "12px" }}
              >
                <Typography.Title style={{ fontSize: "15px", margin: 0 }}>
                  Настройки
                </Typography.Title>
                {!isEditing && (
                  <Typography.Title
            ...
            >
              ...
              {error && (
                <Typography.Body style={{ color: "#FF0000" }}>
                  {error}
                </Typography.Body>
              )}
              {metrics && (
                ...
                >
                  ...
                  <Flex
                    direction="column"
                    style={{ gap: "4px", fontSize: "13px", width: "100%" }}
                  >
                    <div
                      style={{
                        display: "flex",
